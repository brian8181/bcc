 ...
 * @param token_match m
 */
void lexer::print_smatch(const parser::token::token_kind_type& t, boost::smatch m)
{
	...
	INFO(FMT_FG_GREEN << "match " << FMT_RESET << "[ " << FMT_ITALIC  << std::right << match  << FMT_RESET_ITALIC << " ](" << match.size()  << ")" << "{: " << FMT_ITALIC << t << FMT_RESET << " :}");
