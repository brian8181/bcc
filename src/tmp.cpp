 ...
 * @return char
 */
char getc(std::streambuf &sbuf) { return static_cast<char>(sbuf.sgetc()); }
...
int write_buf(const string &file, /* in */ const char *buf, const int &len) {
  std::ofstream stream(file, std::ofstream::out | std::ofstream::binary |
                                 std::ofstream::trunc);
  if (stream.is_open()) {
 ...
 * @return stringstream&
 */
stringstream &read_sstream(const string &file, /* out */ stringstream &ostrm,
                           std::ios_base::openmode mode) {
  ifstream stream(file, mode);
  if (stream.is_open()) {
    char c;
